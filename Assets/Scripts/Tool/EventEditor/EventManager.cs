        ...
        if (Application.isPlaying)
        {
            ...
            foreach (var dic in Data.Instance.GetAllData())
            {
                foreach (var pair in dic.Value)
                {
                    if (dic.Key == "Default")
                    {
                        _conditionEventCreator.SetEnvParam(pair.Value.GetType().Name, pair.Value);
                    }
                    else
                    {
                        _conditionEventCreator.SetEnvParam(dic.Key + "_" + pair.Value.GetType().Name, pair.Value);
                    }
                }
            }
        }
