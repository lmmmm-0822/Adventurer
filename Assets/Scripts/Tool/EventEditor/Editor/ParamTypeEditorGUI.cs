...
namespace EventLogicSystem
{
    ...
    public class EnumParamEditorGUI : ParamTypeEditorGUI
    {
        ...
        public override void OnGUI(string name, string token, Action<string> setToken)
        {
            ...
            EditorGUILayout.LabelField(nameAndType[0]);
            if (!int.TryParse(token, out _))
                token = "0";
            var e = (Enum)Enum.Parse(BaseFunc.String2EnumType[nameAndType[1]], token);
            token = Convert.ToInt32(EditorGUILayout.EnumPopup(e)).ToString();
            setToken(token);
