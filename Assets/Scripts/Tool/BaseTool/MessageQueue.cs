...
namespace QxFramework.Core
{
    ...
    public class MessageQueue<TEventType> : IMessageQueue where TEventType : struct
    {
        ...
        public void RegisterHandler(TEventType msgId, EventHandler<EventArgs> callback)
        {
            ...
            ConsoleProDebug.LogToFilter("注册了关于" + msgId.GetType().Name + "." + msgId.ToString() + "的消息", "Msg");
        ...
        public void DispatchMessage(TEventType msgId, object sender, EventArgs param = null,bool immediate = true)
        {
            if (!_handlerMap.ContainsKey(msgId))
            {
                ConsoleProDebug.LogToFilter("× 此时不存在注册了" + msgId.GetType().Name + "." + msgId.ToString() + "的消息", "Msg");
                return;
            }

            ConsoleProDebug.LogToFilter("o 发送消息" + msgId.GetType().Name + "." + msgId.ToString() , "Msg");
            //消息直接分发或者加入队列
            if (_immediate && immediate)
            {
                if (_handlerMap[msgId].DispatchMessage(sender, param))
                    ConsoleProDebug.LogToFilter("√ 执行了" + msgId.ToString() + "的消息", "Msg");
                else
                    ConsoleProDebug.LogToFilter("× 所有关于" + msgId.ToString() + "的消息都已被移除", "Msg");
            }
            else
            {
                ConsoleProDebug.LogToFilter("       加入了队列", "Msg");
        ...
        public void Update()
        {
            if (_messageQueue.Count>0)
            {
                ...
                {
                    if (_handlerMap[msg.Id].DispatchMessage(msg.Sender, msg.Param))
                        ConsoleProDebug.LogToFilter(" √ 延迟执行了" + msg.Id.GetType().Name + "." + msg.Id.ToString() + "的消息", "Msg");
                }
        ...
        private class MessageHandlerCollection
        {
            ...
            public bool DispatchMessage(object sender, EventArgs param)
            {
                bool tmp = false;
                ...
                for (int i = 0,cnt = list.Count; i < cnt; i++)
                {
                    if (list[i] != null)
                    {
                        //ConsoleProDebug.LogToFilter("执行了发送者为" + sender.GetType().Name + "的消息", "Msg");
