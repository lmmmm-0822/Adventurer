...
public class TableAgent
{
    ...
    /// <param name="key1">The key1.</param>
    /// <param name="key2">The key2.</param>
    /// <param name="nullReturn">不是float，则返回该值</param>
    /// <returns></returns>
    public float TryGetFloat(string name, string key1, string key2,float nullReturn = float.NaN)
    {
        float output;
        ...
        if (float.TryParse(_tableAgent[name][new TableKey(key1, key2)], out output))
        {
            _floats[new TableItemKey(name, key1, key2)] = output;
            return output;
        }
        ...
        {
            _floats[new TableItemKey(name, key1, key2)] = float.NaN;
            return nullReturn;
        }
    }
    ...
    public int GetInt(string name, string key1, string key2)
    {
        ...
        try
        {
            if (_tableAgent[name][new TableKey(key1, key2)] != "")
